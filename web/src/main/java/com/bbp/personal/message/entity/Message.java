 ...
 * 描述：<p>
 * 系统消息<p>
 * 1、收件箱和发件箱内的消息默认365天后被放入垃圾箱<p>
 * 2、垃圾箱内的消息30天后自动物理删除<p>
 * 3、垃圾箱内的消息只有只有当收件人和发件人 把消息都从垃圾箱中删除后才能物理删除<p>
 * 4、收藏箱的不能删除<p><p>
 * 如果type==system_message_all表示是发给所有人的消息 策略如下：<p>
 * 1、首先在展示时（第一页），会会自动查所有的system_message_all<p>
 * 2、如果用户阅读了，直接复制一份 放入它的收件箱 状态改为system_message<p>
 * 如果消息是草稿 那么收件人状态是null<p>
 */
@Entity
